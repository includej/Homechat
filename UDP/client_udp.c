...
void clienthread(int client_r){
    ...
    while(1)
    {
        ...
        if(choice==1){
            ...
            if(sendto(sockfd, &choice, sizeof(choice), 0, (struct sockaddr*)&server_address, server_struct_length) < 0)
            {
            ...
            if(sendto(sockfd, &user, sizeof(user), 0, (struct sockaddr*)&server_address, server_struct_length) < 0)
            {
                printf("Send Failed. Error!!!!\n");
                return -1;
            }
        ...
        /*Login choice*/
            ...
            if(recvfrom(sockfd, &ret, sizeof(ret),0, (struct sockaddr*)&server_address, &server_struct_length) < 0)
            {
    ...
    while(choice!=3)
    {
        ...
        if(choice==4){
            ...
            if(sendto(sockfd, &tmp, sizeof(tmp), 0, (struct sockaddr*)&server_address, server_struct_length) < 0)
            {
                printf("Send Failed. Error!!!!\n");
                return -1;
            }
        ...
        /*Received message choice*/
            ...
            if(recvfrom(sockfd, &rec, sizeof(rec),0, (struct sockaddr*)&server_address, &server_struct_length) < 0)
            {
                printf("Receive Failed. Error!!!!!\n");
                return -1;
            }
            ...
            for(int i = 0; i < rec; i++)
            {
                ...
                if(recvfrom(sockfd, &fr, sizeof(fr),0, (struct sockaddr*)&server_address, &server_struct_length) < 0)
            {
                printf("Receive Failed. Error!!!!!\n");
                return -1;
            }
